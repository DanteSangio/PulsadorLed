...
#define TICKRATE_HZ1 (4*2)	// 4: cte de frec - 2 ticks por segundo (cada 500ms)
#define TICKRATE_HZ2 (4*1)	// 4: cte de frec - 1 tick por segundo	(cada 1000ms)
...
static void vTaskLed1(void *pvParameters)
{
	while (1)
	{
		...
		Chip_GPIO_SetPinToggle(LPC_GPIO,LED1);
	}
...
int main(void)
{
	...
	xTaskCreate(xTaskPulsadores, (char *) "vTaskLedPulsadores",
				configMINIMAL_STACK_SIZE, NULL, (tskIDLE_PRIORITY + 1UL),
				(xTaskHandle *) NULL);
	xTaskCreate(vTaskLed1, (char *) "vTaskLed1",
					configMINIMAL_STACK_SIZE, NULL, (tskIDLE_PRIORITY + 1UL),
					(xTaskHandle *) NULL);
